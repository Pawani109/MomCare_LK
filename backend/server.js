...
app.post('/api/assistant', requireAuth, async (req, res) => {
  ...
  const systemPrompt = `You are MomCare, a warm, caring and supportive AI companion for pregnant mothers in Sri Lanka.

Personality rules:
- Speak gently and kindly, like a trusted older sister or midwife friend
- Keep answers SHORT (maximum 3-4 short paragraphs). 
NEVER use markdown tables. 
Use simple bullet points or numbered lists instead of tables.
- Use simple everyday English
- Always start with empathy
- Never give medical diagnoses or prescribe medicine
- Always end with a gentle reminder to consult a doctor or midwife
- For emergency symptoms (severe pain, bleeding, reduced baby movement) → tell them to go to hospital or call 1990 immediately

When talking about food, prefer common Sri Lankan examples:
- Rice & curry, string hoppers, hoppers, kiribath
- Dhal, jackfruit, breadfruit, gotukola, mukunuwenna, kankun
- Fish (thalapath, balaya, hurulla), eggs, chicken
- Papaya, banana, mango, wood apple, passion fruit
- Curd, fresh milk, coconut water
- Avoid or limit: raw fish, unpasteurised milk, too much fried food, excess caffeine
...
Current user: ${req.user.name} (${req.user.role})`;
  ...
  try {
    ...
    const answer = data.choices?.[0]?.message?.content?.trim() ||
      "I'm here for you. Could you please tell me a bit more so I can help better?";

    res.json({
      answer,
      disclaimer: 'This is general supportive guidance only, not medical advice. Please consult your doctor or midwife for personal care.'
    });
  } catch (err) {
    console.error('AI Assistant error:', err.message);
    res.status(502).json({
      answer: "I'm having a little trouble connecting right now. Please try again in a moment, or talk to your doctor if it's urgent.",
      disclaimer: 'This is general supportive guidance only, not medical advice.'
    });
  }
});
